							...
							if(Object.keys(resp).length > 0){
								...
								Object.keys(resp).map(k=>{
									...
									tr.append('<td class="">'+(resp[k].status == 1 ? 'Paid' :'Unpaid')+'</td>')
										...
										tr.append('<td><center><button class="btn btn-sm btn-primary mr-2 text-white edit_booked" data-id="'+resp[k].schedule_id+'" data-bid="'+resp[k].id+'"><strong>Confirm</strong></button><button class="btn btn-sm btn-danger remove_booked" data-id="'+resp[k].id+'">Delete</button></center></td>')
